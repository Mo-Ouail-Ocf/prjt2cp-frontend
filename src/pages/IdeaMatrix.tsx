...
const IdeaMatrix = () => {
  ...
  const selected = useIdeaStore((state) => state.selected);
  ...
  const handleCombine = (content: string, details: string) => {
    ws?.sendCombinedIdea(content, details, selected);
  };
  ...
  return (
    <div className="w-screen p-4 pr-16 pl-16 flex flex-col justify-around">
      ...
      {isMod && showMultiActions && (
        <MultiActionMenu
          handleAction={handleCombine}
          action="Combine"
          ideaIds={selected}
          disabled={false}
        >
          <Button className="fixed bottom-2 z-10 right-4 w-44 h-12">
            Combine
          </Button>
        </MultiActionMenu>
      )}
